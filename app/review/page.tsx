      ...
      <section className="flex flex-col gap-6 max-w-3xl mx-auto py-2">
        ...
        <header className="flex flex-col gap-3">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-red-500/10 text-red-500 ring-1 ring-red-500/30 shadow-lg shadow-red-950/20">
            ...
            {totalDueCount > 0 && (
              <div className="shrink-0 flex flex-col items-end justify-center pl-2">
                <div className="flex items-baseline gap-1 font-mono">
                  ...
                  <span className="text-xs text-zinc-600 font-light select-none">/</span>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-28 text-zinc-500">
            <Loader2 className="h-6 w-6 animate-spin text-red-500/80" />
            <span className="mt-3 text-xs font-mono text-zinc-400">Loading due reviews...</span>
          </div>
        ) : totalDueCount === 0 || !currentRecord ? (
          /* Completion State */
          <article className="relative overflow-hidden flex flex-col items-center justify-center rounded-2xl sm:rounded-3xl border border-zinc-800/80 bg-zinc-900/50 p-8 sm:p-12 text-center shadow-2xl backdrop-blur-xl">
            <div className="absolute -top-24 left-1/2 -translate-x-1/2 w-64 h-64 bg-red-500/10 rounded-full blur-3xl pointer-events-none" />

            <div className="relative flex h-16 w-16 items-center justify-center rounded-2xl bg-zinc-800/80 text-zinc-100 ring-1 ring-zinc-700/50 shadow-xl mb-5">
              <Sparkles className="h-7 w-7 text-red-400 animate-pulse" />
            </div>

            <span className="rounded-full bg-red-500/10 border border-red-500/20 px-3 py-0.5 text-xs font-semibold text-red-400 font-mono mb-2">
              Queue Cleared
            </span>

            <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-white">All Caught Up!</h2>
            ...
            {completedInSession > 0 && (
              <div className="mt-5 inline-flex items-center gap-2 rounded-full bg-zinc-950/80 border border-zinc-800 px-4 py-1.5 text-xs font-mono text-zinc-300">
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
              <Link
                href="/random"
                className="flex items-center gap-1.5 rounded-full bg-white px-4 py-1.5 text-xs font-semibold text-zinc-950 hover:bg-zinc-200 transition-all active:scale-95 shadow-sm shadow-white/10"
              >
              ...
              <Link
                href="/"
                className="flex items-center gap-1.5 rounded-full border border-zinc-800 bg-zinc-900/90 px-4 py-1.5 text-xs font-medium text-zinc-300 hover:text-white hover:border-zinc-700 hover:bg-zinc-800 transition-all active:scale-95"
              >
          ...
          <article className="flex flex-col rounded-2xl sm:rounded-3xl border border-zinc-800/80 bg-zinc-900/50 shadow-2xl backdrop-blur-xl overflow-hidden transition-all">
            {/* Top ambient highlight line */}
            <div className="h-px w-full bg-gradient-to-r from-transparent via-white/10 to-transparent" />

            {/* Hero Cover Media Banner */}
            {currentRecord.thumbnail_url && (
              <div className="relative h-44 sm:h-64 w-full overflow-hidden border-b border-zinc-800/80">
                <Image
            ...
            <div className="p-5 sm:p-7 flex flex-col gap-5">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  {currentRecord.tags && currentRecord.tags.length > 0 ? (
                    currentRecord.tags.map((tag) => (
                      <span
                        key={tag.id}
                        className="inline-flex items-center rounded-full border border-zinc-700/50 bg-zinc-800/80 px-2.5 py-0.5 text-[10px] font-medium text-zinc-300 font-mono"
                      >
                        #{tag.name}
                      </span>
                    ))
                ...
                <div className="flex items-center gap-1 p-1 rounded-xl bg-zinc-950/70 border border-zinc-800/80 text-[10px] font-mono self-start sm:self-auto">
              ...
              {currentRecord.source_url && getYouTubeVideoId(currentRecord.source_url) && (
                <div className="overflow-hidden rounded-2xl border border-zinc-800/80 shadow-lg">
              ...
              <div className="border-t border-zinc-800/80 pt-4">
                {!isContentRevealed ? (
                  <div className="flex flex-col gap-3 rounded-2xl border border-zinc-800/80 bg-zinc-950/60 p-4 transition-all">
                    ...
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-xl bg-zinc-900/90 hover:bg-zinc-800 py-3 px-4 text-xs font-semibold text-zinc-200 hover:text-white transition-all active:scale-[0.99] shadow-sm border border-zinc-800 group cursor-pointer"
                    >
                  ...
                  <div className="flex flex-col gap-3 rounded-2xl border border-zinc-800/80 bg-zinc-950/90 p-5 sm:p-6 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="flex items-center justify-between border-b border-zinc-800/80 pb-3">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-medium text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/60 transition-colors cursor-pointer border border-zinc-800/80"
                      >
                        <span>Hide</span>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3 pt-1">
                ...
                <button
                  ...
                  className="group flex h-9 sm:h-10 items-center justify-center gap-1.5 rounded-full border border-red-500/30 bg-red-500/10 px-2 sm:px-3 text-xs font-semibold text-red-300 transition-all hover:bg-red-500/20 hover:border-red-500/50 active:scale-95 disabled:opacity-50 cursor-pointer shadow-sm"
                >
                ...
                <Link
                  href={`/records/${currentRecord.id}`}
                  className="group flex h-9 sm:h-10 items-center justify-center gap-1.5 rounded-full border border-zinc-800 bg-zinc-900/90 px-2 sm:px-3 text-xs font-semibold text-zinc-300 transition-all hover:bg-zinc-800 hover:text-white hover:border-zinc-700 active:scale-95 shadow-sm cursor-pointer"
                >
                ...
                <button
                  ...
                  disabled={reviewMutation.isPending}
                  className="group flex h-9 sm:h-10 items-center justify-center gap-1.5 rounded-full bg-white px-2 sm:px-3 text-xs font-bold text-zinc-950 transition-all hover:bg-zinc-200 active:scale-95 shadow-sm shadow-white/10 disabled:opacity-50 cursor-pointer"
                >
                  <Check className="h-3.5 w-3.5 shrink-0 stroke-[2.5] text-zinc-950 transition-transform group-hover:scale-110" />
